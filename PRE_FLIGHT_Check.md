...
A documentation-only branch-control mistake created an intermediate commit with `ACTIVE_CONTEXT.md` replaced by a placeholder. The corrective merge uses the already-reviewed v0.2.2 candidate tree, restores the proper router, records this correction here and in `CHANGELOG.md`, and avoids force-rewriting Dev history. Runtime source and Stable were not modified by the accidental commit.
