...
# Strong invariant: validated provider/capture functions remain byte-identical.
...
for forbidden in ['function buildUI(', 'function registerTool(', 'kwPBResRoot', 'kwPBResButtons', 'kwPBResEnable']:
    if forbidden in service:
        raise RuntimeError(f'Service still contains presentation ownership: {forbidden}')

service_path.write_text(service)

# UI becomes explicit sole presentation owner; presentation remains otherwise unchanged.
...
if old not in text:
    raise RuntimeError('Expected MASTER High Res sentence not found')
