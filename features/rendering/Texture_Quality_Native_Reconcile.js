  ...
  function diagnosticSessionState() {
    ...
    return {
      ...
      figures: session.pipelines.map((p) => ({
        ...
        aaids: p.aaids ? Object.fromEntries(BODIES.map((key) => {
          const ref = p.aaids[key];
          return [key, ref ? {
            required: !!ref.required,
            path: ref.path,
            size: ref.size,
            texture: diagnosticTextureRef(ref.texture)
          } : null];
        })) : null,
        ...
        trackedSnapshots: {
          ...
          aaidWrappers: Array.isArray(p.aaidLookupWrappers) ? p.aaidLookupWrappers.length : 0
        }
      }))
    };
  }
