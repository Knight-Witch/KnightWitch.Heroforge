...
Issue #58 — Bundle Polymorph display fonts for Witch Dock headers — is the active Dev task. Amanda's latest visual direction narrows Polymorph to the main brand title only: larger letter-spaced `WITCH [emblem] DOCK`, centered within the header space left of the controls, with small muted Dev/version metadata immediately after the title. Tabs, section/tool headers, modal titles, buttons, and labels use the existing readable system stack with lighter weights; section/tool/modal headers are larger, slightly letter-spaced, and uniformly uppercase. Polymorph Regular and Bold remain bundled so the title weight can be changed later without another asset recovery. Candidate module revisions are Core v2.3.0, Shell v0.5.1, and Styles v0.5.1. Dev v1.9.3 / `1.9.3-title-alignment-fix` is pinned to immutable payload `47208351d47baec3cc0fa08d6446b37e1b2261c4`. Live Dev verification passed: Auto Host resolved exact canonical head `024602202fc5a2bc77393529a57b30047ab3a54a`; loader passed 25/25 with 0 failed and 0 fallback; Polymorph Bold loaded from the immutable payload; the 20px emblem, inline muted `DEV · v1.9.3`, and title/control geometry are present with a 14px gap before the window controls. Public Stable remains v2.2.2; only Amanda's visual approval remains before any promotion.
