...
Issue #58 — Bundle Polymorph display fonts for Witch Dock headers — is the active Dev task. Amanda's latest visual direction narrows Polymorph to the centered main brand title only: larger letter-spaced `WITCH [emblem] DOCK`, with a small muted Dev/version line underneath that does not increase header height. Tabs, section/tool headers, modal titles, buttons, and labels use the existing readable system stack with lighter weights; section/tool/modal headers are larger, slightly letter-spaced, and uniformly uppercase. Polymorph Regular and Bold remain bundled so the title weight can be changed later without another asset recovery. Candidate module revisions are Core v2.3.0, Shell v0.5.0, and Styles v0.5.0. Dev v1.9.2 / `1.9.2-title-typography-refine` is pinned to immutable payload `766641726eb785f51c8c6af342341b089cd46868`. Public Stable remains v2.2.2; next gates are live Dev verification and Amanda's visual approval.
