...
// @run-at       document-end
// @updateURL    https://raw.githubusercontent.com/Knight-Witch/KnightWitch.Heroforge/WITCH_DEV_UI/Witch_Dock_DEV.user.js
// @downloadURL  https://raw.githubusercontent.com/Knight-Witch/KnightWitch.Heroforge/WITCH_DEV_UI/Witch_Dock_DEV.user.js
// @grant        unsafeWindow
// @grant        unsafeWindow
// @grant        GM_addStyle
...
function kwWithCacheKey(url, key, value) {
  try {
...
function kwModuleCacheKey(manifest, tool) {
  const registry = Array.isArray(manifest && manifest.moduleRegistry) ? manifest.moduleRegistry : [];
  const meta = registry.find((entry) => entry && entry.id === tool.id) || null;
  const identity = [
    tool.id || "",
    tool.url || "",
    (meta && meta.path) || "",
  ...
  return kwStableHash(identity);
}

const LOADER_DIAGNOSTICS = {
  version: "0.6.0",
  build: "1.0.8.6-cache-keyed-loader",
  sessionToken: LOADER_CACHE_SESSION,
  manifestBaseURL: MANIFEST_URL,
  manifestRequestURL: null,
  moduleRequests: []
};
UW.KWWitchDockLoaderCache = LOADER_DIAGNOSTICS;
...
async function loadManifestAndTools() {
  let manifest;
  try {
    const manifestRequestURL = kwWithCacheKey(MANIFEST_URL, "kwd_manifest", LOADER_CACHE_SESSION);
    LOADER_DIAGNOSTICS.manifestRequestURL = manifestRequestURL;
    UW.KWWitchDockManifestRequestURL = manifestRequestURL;
    const raw = await gmGetText(manifestRequestURL);
  ...
  for (const t of tools) {
    ...
    try {
      const moduleRequestURL = kwWithCacheKey(url, "kwd_module", kwModuleCacheKey(manifest, t));
      LOADER_DIAGNOSTICS.moduleRequests.push({ id, baseURL: url, requestURL: moduleRequestURL });
      const code = await gmGetText(moduleRequestURL);
      if (!code) continue;
      new Function(code)();
    } catch (e) {}
  }
