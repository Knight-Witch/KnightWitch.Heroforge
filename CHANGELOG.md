...
Two accidental documentation-only commits temporarily replaced `ACTIVE_CONTEXT.md` with a one-character placeholder while advancing Dev. The corrective merge restores the reviewed v0.2.2 candidate tree without force-rewriting branch history. Neither accidental commit changed any runtime file or the Stable branch.
