// ==UserScript==
// @name         Body Editor - RETIRED (standalone userscript for manual install)
// @namespace    http://tampermonkey.net/
// @version      0.2.0
// @description  Body Editor tool registered into Witch Dock (WITCH DOCK TEST v3).
// @match        https://www.heroforge.com/*
// @match        https://heroforge.com/*
// @grant        unsafeWindow
// @run-at       document-idle
// @updateURL    https://raw.githubusercontent.com/Knight-Witch/KnightWitch.Heroforge/Witch_Scripts/tools/archive/Body_Editor.user.js
// @downloadURL  https://raw.githubusercontent.com/Knight-Witch/KnightWitch.Heroforge/Witch_Scripts/tools/archive/Body_Editor.user.js
// ==/UserScript==
...
(function () {
  ...
  const state = {    arms: { target2: true, target3: true, copyPos: true, copyQtn: true, copyScl: true, syncHands: true },
  ...
  function loadState() {
    try {
      ...
      if (!saved || typeof saved !== 'object') return;      if (saved.arms && typeof saved.arms === 'object') Object.assign(state.arms, saved.arms);
  ...
  function renderArmsSection(container, api) {
    const s = api.ui.createSection({ id: 'arms', title: 'Extra Arms Sync', defaultCollapsed: false });
    const body = s.body;
  ...
  function renderBreastSection(container, api) {
    const s = api.ui.createSection({ id: 'breast', title: 'Breast Mirror', defaultCollapsed: false });
    const body = s.body;
  ...
  function renderButtSection(container, api) {
    const s = api.ui.createSection({ id: 'butt', title: 'Butt Mirror', defaultCollapsed: false });
    const body = s.body;
  ...
  function renderTool(container, api) {
    ...
    renderArmsSection(rootEl, api);
    renderBreastSection(rootEl, api);
    renderButtSection(rootEl, api);
  ...
  function registerIntoWitchDock() {
    ...
    wd.registerTool({
      id: 'body-editor',
      tab: 'Body Editor',
      title: 'Body Editor WITCH DOCK TEST v3',
